...
echo "For NASA_Jul95.log, the top 10 websites which requests came (non-404 status):"
awk '$9 != 404 {print $1}' NASA_Jul95.log | sort | uniq -c | sort -nr | head
echo "\n"
echo "For NASA_Aug95.log, the top 10 websites which requests came (non-404 status):"
...
echo "For NASA_Jul95.log, here are the most frequent request types."
...
echo "For NASA_Jul95.log, here are the number of requests at each hour"
awk '{split($4, a, ":"); print a[2]}' NASA_Jul95.log | sort | uniq -c 
echo "Based on this information, it seems that the most active time of day is around noon between 10:00 - 16:00."
echo "The quietest period seems to be around the middle of the night between 02:00 - 07:00."

echo "\nFor NASA_Aug95.log, here are the number of requests at each hour"
awk '{split($4, a, ":"); print a[2]}' NASA_Aug95.log | sort | uniq -c 
echo "Based on this information, it seems that the most active time of day is around afternoon between 12:00 - 16:00."
echo "The quietest period seems to be around the middle of the night between 01:00 - 05:00."
...
echo "Let's first see if there are any missing dates."
awk '{print substr($4, 2, 11)}' NASA_Aug95.log | cut -d: -f1 | sort | uniq
echo "We can see that there is no data for August 2nd. We can get the last log from August 1st and the first log from August 3rd to get a good approximation for the hurricane."
awk '$4 ~ /01\/Aug\/1995/ {gsub(/\[/, "", $4); print $4 }' NASA_Aug95.log | sort | tail -1
awk '$4 ~ /03\/Aug\/1995/ {gsub(/\[/, "", $4); print $4 }' NASA_Aug95.log | sort | head -1
echo "The hurricane from August caused an outage that potentially lasted from August 1st, 1995 at 14:52:01 until August 3rd, 1995 at 04:36:13, lasting a total of 1 day, 13 hours, 44 minutes, and 12 seconds."
...
echo "July 13th, 1995 saw the most amount of activity for NASA_Jul95.log."

echo "\nFor NASA_Aug95.log: "
awk '{print substr($4, 2, 11)}' NASA_Aug95.log | cut -d: -f1 | sort | uniq -c | sort -nr | head -1
echo "August 31st, 1995 saw the most amount of activity for NASA_Aug95.log."

echo "\n---\n"

echo "11. Excluding the outage dates which date saw the least amount of activity?\n"

echo "For NASA_Jul95.log: "
awk '{print substr($4, 2, 11)}' NASA_Jul95.log | cut -d: -f1 | sort | uniq -c | sort -nr | tail -5
echo "July 28th, 1995 saw the least amount of activity for NASA_Jul95.log."

echo "\nFor NASA_Aug95.log: "
awk '{print substr($4, 2, 11)}' NASA_Aug95.log | cut -d: -f1 | sort | uniq -c | sort -nr | tail -5
echo "August 26th, 1995 saw the least amount of activity for NASA_Aug95.log."
